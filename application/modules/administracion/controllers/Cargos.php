...
class Cargos extends Admin_Controller
{
    ...
    {
        $data['js_data'] = array(
            base_url() . 'assets/custom/cargos.js',
            //  base_url().'assets/plugins/select2/select2.full.min.js',
        );

        $data['page'] = $this->config->item('gpm_sys_web_template_dir_admin') . "cargos_lista";
        $data['modals'] = array(
            $this->load->view($this->config->item('gpm_sys_web_template_dir_admin') . "cargos_registrar", '', TRUE),
        );
